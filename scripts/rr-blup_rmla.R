...
# data base
dbfile <- "sqldb/hs_rrblup.sqldb" # database
...
# file names
fname1 <- paste0(st.input.dir, "/hs-marker-") # marker data
fname2 <- "hs-marker-" # marker data
...
marker <- read.table("data/haploselekt-beagle-out.mpo", header=T)
    ...
    for (TRS in transformation) {
      ...
      st.read.performance.data(fname3, data.set="vv") # phenotypic data for VS
        ...
        for (EST in estimation){
          ...
          rfile.yy <- paste0("yhat_", EST, "_snp_", TRS, "_", TRT )
